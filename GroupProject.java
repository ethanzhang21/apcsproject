...
public class GroupProject {
    ...
    public void handleClick(int row, int col) {
        if (selectedRow == -1 && board[row][col] != null) {
            ...
            if (selected.canMove(row, col, board) && selected.isWhite == isWhiteTurn) {
...
class Pawn extends Piece {
    ...
    public boolean canMove(int newX, int newY, Piece[][] board) {
        ...
        if (newX == x + dir && Math.abs(newY - y) == 1 && board[newX][newY] != null && board[newX][newY].isWhite != this.isWhite)
            return true;
